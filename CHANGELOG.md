...
- 🔑 **API Key Generation Support**: Generate secret keys to leverage NeuroHog with OpenAI libraries.
...
- **🔗 Built-in LiteLLM Proxy**: Now includes LiteLLM proxy within NeuroHog for enhanced functionality.

  - Easily integrate existing LiteLLM configurations using `-v /path/to/config.yaml:/app/backend/data/litellm/config.yaml` flag.
  - When utilizing Docker container to run NeuroHog, ensure connections to localhost use `host.docker.internal`.
...
- Ollama is no longer required to run NeuroHog.
- Access our comprehensive documentation at [NeuroHog Documentation](https://docs.openwebui.com/).
