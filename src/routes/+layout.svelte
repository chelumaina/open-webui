		...
		if (backendConfig) {
			...
			if ($config) {
				...
				if (localStorage.token) {
					...
					// alert("Current Path: "+str);
					if (str == '/') 
					{
						// alert("Redirecting to /landing-page");
						await goto(`/landing-page`);
					}
					else 
					{
						// Do nothing if already on /auth
						const keywords = ["/landing-page","/auth", "/activate", "/help", "/error", "/features", "/pricing", "/terms", "/privacy", "/sitemap.xml", "/robots.txt", "/about"];
...
{#if loaded && !$isApp}
	<footer class="w-full py-6 px-8 bg-gray-50 dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800">
		<div class="max-w-7xl mx-auto">
			<nav class="flex flex-wrap justify-center gap-6 mb-4">
				<a href="/" class="text-sm text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white transition-colors">
					Home
				</a>
				<a href="/features" class="text-sm text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white transition-colors">
					Features
				</a>
				<a href="/help" class="text-sm text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white transition-colors">
					Help
				</a>
